...
### firewall (`src/sources/firewall.rs`)
...
### logtamper (`src/sources/logtamper.rs`)
...
### network (`src/sources/network.rs`)
...
### journald (`src/sources/journald.rs`)
...
### samhain (`src/sources/samhain.rs`)
...
### cognitive (`src/detect/cognitive.rs`)
...
### barnacle (`src/detect/barnacle.rs`)
...
The aggregator (`src/core/aggregator.rs`) sits between raw sources and consumers, preventing alert storms from overwhelming Slack or the TUI.
...
The terminal dashboard (`src/tui/mod.rs`) uses ratatui/crossterm and provides six tabs.
...
4. **Aggregator dedup windows** — The default 30-second dedup window suppresses repeated identical alerts. Scan-prefixed sources use a 1-hour window. To change, modify `AggregatorConfig::default()` in `src/core/aggregator.rs`.
