# ClawAV v0.1.9 Implementation Plan
...
**Architecture:** Surgical changes across existing modules. Fuzzy dedup in aggregator, auid extraction in auditd, Ed25519 signing in update pipeline, config-driven network allowlists, compiler/linker allowlisting in behavior, health endpoint + periodic Slack heartbeat, alert persistence to JSONL, SSH monitoring via journald, scan timeouts, `clawav config` CLI, and docs.

**Tech Stack:** Rust, ed25519-dalek, ipnet (new deps), tokio, serde, existing ClawAV architecture

**Current version:** 0.1.8 → target 0.1.9
...
Change `version = "0.1.8"` to `version = "0.1.9"` in `Cargo.toml`.
...
- name: Sign release binaries
  ...
  run: |
    ...
    for binary in target/*/release/clawav; do
      # Create signature
...
## Task 11: `clawav config` CLI

Add `clawav config get` and `clawav config set` subcommands.
...
"config" => {
    match args.get(2).map(|s| s.as_str()) {
        ...
        Some("set") => {
            let field = args.get(3).context("Usage: clawav config set <field> <value>")?;
            let value = args.get(4).context("Usage: clawav config set <field> <value>")?;
            ...
            println!("Restart clawav for changes to take effect.");
        }
        _ => {
            eprintln!("Usage: clawav config [get|set] [field] [value]");
        }
    }
    return Ok(());
}
...
```bash
git add src/main.rs
git commit -m "feat: clawav config get/set CLI commands"
```
...
- Slack webhook not working → test with `clawav test-webhook`
- High CPU usage → reduce scan frequency
- False positives from cargo build → explained and fixed in v0.1.9
...
- `clawav config get/set` CLI commands
...
git commit -m "docs: add configuration reference, troubleshooting, and changelog for v0.1.9"
```
...
```bash
./target/release/clawav --help
./target/release/clawav config get
```

**Step 4: Tag release**

```bash
git tag -a v0.1.9 -m "v0.1.9: noise reduction, attribution, signing, monitoring"
