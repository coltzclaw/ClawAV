...
The scanner lives in `src/scanner/mod.rs`. On a configurable interval, `run_periodic_scans()` spawns a blocking task that calls `SecurityScanner::run_all_scans()`, which invokes every `scan_*` function. Results are:
...
#### `scan_cognitive_integrity()` *(in `src/detect/cognitive.rs`)*
...
#### `scan_audit_log_health()` *(in `src/sources/logtamper.rs`)*
...
The BarnacleDefense engine (`src/detect/barnacle.rs`) loads regex-based threat patterns from JSON files and compiles them at startup for fast matching.
...
1. **Add the function** in `src/scanner/mod.rs`:
