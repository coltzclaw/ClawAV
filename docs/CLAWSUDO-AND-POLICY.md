...
> **Note:** clawsudo has its own independent policy loader (`src/bin/clawsudo.rs`) that only supports `command` and `command_contains` match criteria. The detection engine (`src/detect/policy.rs`) supports all five criteria above.
