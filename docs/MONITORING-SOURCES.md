...
ClawTower monitors Unix systems through nine real-time data sources, each implemented as an async task that feeds alerts into a unified channel. This document covers every source, how it works internally, and how to add new ones.
...
Create `src/sources/mymonitor.rs`:
...
pub async fn tail_my_source(
    ...
    // Option A: Tail a file (see sources/auditd.rs, sources/falco.rs, sources/samhain.rs)
    // Option B: Spawn a child process (see sources/journald.rs)
    // Option C: Poll a command (see sources/firewall.rs)
    loop {
...
tokio::spawn(async move {
    if let Err(e) = crate::sources::mymonitor::tail_my_source(tx_clone).await {
