...
main.rs
├── config/
│   ├── mod.rs           (Config, all *Config structs)
│   ├── merge.rs         (TOML overlay merge engine)
│   ├── cloud.rs         (Cloud provider config)
│   ├── openclaw.rs      (OpenClaw integration config)
│   └── export.rs        (SIEM export pipeline)
├── core/
...
│   ├── orchestrator.rs  (Task orchestration and wiring)
│   ├── response.rs      (Response engine and playbooks)
│   ├── risk_latch.rs    (Risk level state machine)
...
│   ├── forensics.rs     (Forensic data collection)
│   ├── prompt_firewall.rs (Prompt injection firewall)
│   ├── detector_runner.rs (Detector abstraction runner)
│   └── registry.rs      (Detector registry)
├── behavior/            (classify_behavior — uses ParsedEvent)
├── interface/
│   ├── api.rs           (HTTP server, AlertRingBuffer, SharedAlertStore)
│   ├── slack.rs         (SlackNotifier — independent webhook)
│   └── tui_client.rs    (TUI client connection)
├── tui/                 (Ratatui dashboard — consumes Alert stream)
├── scanner/             (periodic security scans)
├── sentinel/            (real-time file integrity via inotify)
├── proxy/               (LLM API proxy with key mapping + DLP)
├── agent/
│   ├── profile.rs       (Agent profile management)
│   ├── envelope.rs      (Agent envelope signing)
│   ├── identity.rs      (Agent identity registry)
│   └── auth_hooks.rs    (Authentication hooks)
├── enforcement/
│   ├── capabilities.rs  (Platform capability detection)
│   ├── process_cage.rs  (Process sandboxing)
│   ├── seccomp.rs       (Seccomp BPF filters)
│   └── apparmor.rs      (AppArmor profile management)
├── safe/
│   ├── safe_cmd.rs      (Safe command execution)
│   ├── safe_io.rs       (Safe file I/O operations)
│   ├── safe_match.rs    (Safe string matching)
...
The auditd parser (`src/sources/auditd.rs`) handles three record types:
...
The behavior engine (`src/behavior/mod.rs`) applies hardcoded rules in priority order:
...
The aggregator (`src/core/aggregator.rs`) sits between all sources and all consumers.
